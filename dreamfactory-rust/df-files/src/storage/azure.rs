...
impl AzureBlobStorageProvider {
    ...
    pub fn new(config: FileServiceConfig) -> FileResult<Self> {
        let azure_config = match &config.provider_config {
            ProviderConfig::AzureBlob(azure_config) => azure_config.clone(),
            _ => {
                return Err(FileServiceError::ConfigError {
                    message: "Invalid provider config for Azure Blob storage".to_string(),
                })
            }
        };
    ...
    /// Get the Azure blob name for a given path
    fn get_blob_name(&self, path: &str) -> String {
    ...
    /// Convert blob name back to relative path
    fn get_relative_path(&self, blob_name: &str) -> String {
        if self.config.root_path.is_empty() || self.config.root_path == "/" {
            format!("/{}", blob_name)
        } else {
            let root = self.config.root_path.strip_prefix('/').unwrap_or(&self.config.root_path);
            if let Some(relative) = blob_name.strip_prefix(&format!("{}/", root)) {
                format!("/{}", relative)
            } else if blob_name == root {
                "/".to_string()
            } else {
                format!("/{}", blob_name)
            }
    ...
    /// Create FileInfo from Azure blob metadata
    fn create_file_info_from_blob(
        &self,
        path: &str,
        size: i64,
...
impl StorageProvider for AzureBlobStorageProvider {
    async fn initialize(&mut self) -> FileResult<()> {
        ...
        // Initialize Azure Blob Storage client using object_store
        let mut builder = MicrosoftAzureBuilder::new()
            .with_account(&self.azure_config.account_name)
            .with_container_name(&self.azure_config.container_name);
        ...
        if let Some(endpoint_url) = &self.azure_config.endpoint_url {
        ...
        self.object_store = Some(Arc::new(builder.build().map_err(|e| {
            FileServiceError::ConfigError {
                message: format!("Failed to initialize Azure Blob client: {}", e),
            }
        })?));
    ...
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            ...
            directory_operations: false, // Azure Blob doesn't have true directories
            metadata_support: true,
            permissions_support: false, // ACLs are separate
            encryption_support: true,
            compression_support: false,
            versioning_support: true, // If blob versioning is enabled
            presigned_urls: true,
            ...
            max_file_size: Some(4 * 1024 * 1024 * 1024 * 1024), // 4TB for block blobs
    ...
    async fn health_check(&self) -> FileResult<ProviderHealth> {
        ...
        if !self.is_ready() {
            return Ok(ProviderHealth {
                ...
                checked_at: Utc::now(),
                response_time_ms: 0,
                error_message: Some("Azure Blob client not initialized".to_string()),
                details: HashMap::new(),
            });
        }
        ...
        Ok(ProviderHealth {
            ...
            error_message: if healthy {
                ...
                Some("Container not accessible".to_string())
            },
            details: HashMap::new(),
        })
    }
...
#[async_trait]
impl FileService for AzureBlobStorageProvider {
    fn service_info(&self) -> FileServiceInfo {
        FileServiceInfo {
            ...
            limits: ServiceLimits {
                max_file_size: Some(4 * 1024 * 1024 * 1024 * 1024), // 4TB
    ...
    async fn upload_bytes(
        ...
        let store = self.object_store.as_ref().unwrap();
        let object_path = ObjectPath::from(blob_name.clone());
    ...
    async fn copy_file(
        ...
        let source_blob = self.get_blob_name(&normalized_source);
        let dest_blob = self.get_blob_name(&normalized_dest);
        ...
        let store = self.object_store.as_ref().unwrap();
        let source_path = ObjectPath::from(source_blob.clone());
        let dest_path = ObjectPath::from(dest_blob.clone());
    ...
    async fn move_file(
        ...
        // Azure Blob doesn't have a native move operation, so we copy then delete
    ...
    // Azure Blob doesn't have true directories, so these operations are simplified
    async fn create_directory(&self, path: &str) -> FileResult<FileOperationResult> {
        let normalized_path = self.validate_path(path)?;
        // In Azure Blob, we can create a "directory" by creating a zero-byte blob with a trailing slash
        let directory_blob = format!("{}/", self.get_blob_name(&normalized_path));
    ...
    async fn list_directory(
        ...
        let prefix = if normalized_path == "/" {
            ...
            Some(ObjectPath::from(format!("{}/", self.get_blob_name(&normalized_path))))
        };
    ...
    async fn delete_directory(
        ...
        if !recursive {
            ...
            let directory_blob = format!("{}/", self.get_blob_name(&normalized_path));
            return self.delete_file(&self.get_relative_path(&directory_blob)).await;
        }

        // For recursive, list and delete all blobs with the prefix
        let prefix = Some(ObjectPath::from(format!("{}/", self.get_blob_name(&normalized_path))));
    ...
    async fn copy_directory(
        ...
        Err(FileServiceError::InternalError {
            message: "Directory copy not implemented for Azure Blob".to_string(),
        })
    }
    ...
    async fn upload_part(
        ...
        // For Azure Blob, we'll store parts temporarily and combine them during completion
    ...
    async fn complete_multipart_upload(
        ...
        // For this simplified implementation, we'll just create an empty blob
        // In a real implementation, you would combine the parts
        let blob_name = self.get_blob_name(&upload.path);
        let object_path = ObjectPath::from(blob_name);
    ...
    async fn batch_operation(&self, operation: BatchOperation) -> FileResult<BatchOperationResult> {
        ...
        // This is a simplified implementation
        Err(FileServiceError::InternalError {
            message: "Batch operations not fully implemented for Azure Blob".to_string(),
        })
    }

    async fn get_storage_usage(&self, _path: Option<&str>) -> FileResult<StorageUsage> {
        // Azure Blob doesn't provide direct storage usage information
        // This would require listing all blobs and summing sizes
        Ok(StorageUsage {
            total_bytes: None,
            used_bytes: 0,
    ...
    async fn search_files(&self, criteria: SearchCriteria) -> FileResult<Vec<FileInfo>> {
        // Implement search using Azure Blob list operations with prefix filtering
        // This is a simplified implementation
    ...
    async fn generate_presigned_url(
        ...
        // Azure Blob presigned URLs would require using SAS tokens
        // This is a simplified implementation
        Err(FileServiceError::InternalError {
            message: "Presigned URLs not fully implemented for Azure Blob".to_string(),
        })
    }
    ...
    async fn set_metadata(
        ...
        // Azure Blob requires copying the blob to update metadata
...
impl AzureBlobStorageProvider {
    /// Apply file filters (simplified for Azure Blob)
...
mod tests {
    ...
    fn test_azure_provider_creation() {
        ...
        let provider = AzureBlobStorageProvider::new(config);
    ...
    fn test_blob_name_generation() {
        ...
        let provider = AzureBlobStorageProvider::new(config).unwrap();

        assert_eq!(provider.get_blob_name("/test.txt"), "test.txt");
        assert_eq!(provider.get_blob_name("test.txt"), "test.txt");
        assert_eq!(provider.get_blob_name("/dir/test.txt"), "dir/test.txt");
    }

    #[test]
    fn test_blob_name_with_root_path() {
        let mut config = FileServiceConfig::azure_blob(
            "test".to_string(),
            "testaccount".to_string(),
            "testcontainer".to_string(),
        );
        ...
        assert_eq!(provider.get_blob_name("/test.txt"), "app/files/test.txt");
        assert_eq!(provider.get_blob_name("test.txt"), "app/files/test.txt");
    }
    ...
    fn test_service_info() {
        ...
        assert_eq!(info.provider, "azure_blob");
    ...
    fn test_capabilities() {
        ...
        assert!(!caps.directory_operations); // Azure Blob doesn't have true directories
    }
}